...
public class BeatBoxParent : MonoBehaviour
{
    ...
    [FormerlySerializedAs("numGroups")]
    public int numGroupsPerLayer = 8;
    ...
    private void Start()
    {
        ...
        for (int verticalLayer = 0; verticalLayer < verticalLayerCount; verticalLayer++)
        {
            ...
            for (int group = 0; group < numGroupsPerLayer; group++)
            {
                ...
                for (int bar = 0; bar < numBarsPerGroup; bar++)
                {
                    Vector3 waveSpawnLocation =
                        spawnLocation + Mathf.Sin(rowCount / 2f) * 3f * Vector3.up;
                    rowCount++;

                    // create box
                    GameObject instantiatedBeatBox = Instantiate(
                        beatBoxPrefab,
                        subParent.transform,
                        true
                    );
                    instantiatedBeatBox.transform.position = transform.transform.TransformPoint(
                        useWavePattern ? waveSpawnLocation : spawnLocation
                    );
                    instantiatedBeatBox.transform.eulerAngles = new Vector3(0, 0, boxRotation);

                    instantiatedBeatBox
                        .GetComponent<BeatBox>()
                        .Initialize(
                            indexArray[bar],
                            maxHeightPerLayer,
                            heightBoostMultiplier,
                            minHeightBooster,
                            backdropHeightMultiplier
                        );
            ...
            spawnLocation =
                originalSpawnLocation + verticalLayer * distanceBetweenVerticalLayers * Vector3.up;
        }
