...
public class RadialGrapple : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.Q))
        {
            ...
            var Dir = new Vector2(1, 1.4f);
            if (transform.localScale.x > 0.5) Dir.x = -Dir.x;

            if (CharController.Instance.isInverted) Dir.y = -Dir.y;
            LaunchHook(Dir);
        }
    ...
    private void LaunchHook(Vector2 direction)
    {
        ...
        var offset = CharController.Instance.isInverted
            ? new Vector3(0, -1, 0)
            : new Vector3(0, 1, 0);
    ...
    public void StartGrapple(Vector3 grapplePoint)
    {
        ...
        var savedVelMag = rigidbody2d.velocity.magnitude;
        ...
        if (transform.localScale.x > 0.5)
        {
            ...
            if (rigidbody2d.velocity.x > -minVel) rigidbody2d.velocity = Vector2.left * minVel;
            //Debug.Log("left boost");
        }
        else if (transform.localScale.x < -0.5)
        {
            rigidbody2d.velocity -= new Vector2(savedVelMag, 0) * gravModifier;
            if (rigidbody2d.velocity.x < minVel) rigidbody2d.velocity = Vector2.right * minVel;
